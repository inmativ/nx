...
describe('Linter', () => {
  ...
  describe('Integrated', () => {
    ...
    describe('linting errors', () => {
      ...
      it('lint plugin should ensure module boundaries', () => {
        ...
        runCLI(`generate apps/@nx/react:app ${myapp2}`);
        runCLI(`generate libs/@nx/react:lib ${lazylib}`);
        runCLI(
    ...
    describe('workspace boundary rules', () => {
      ...
      beforeAll(() => {
        ...
        runCLI(
          `generate @nx/js:lib libs/${libA} --linter=eslint --bundler=none`
        );
        runCLI(
          `generate @nx/js:lib libs/${libB} --linter=eslint --bundler=none`
        );
        runCLI(
          `generate @nx/js:lib libs/${libC} --linter=eslint --bundler=none`
        );
  ...
  describe('Root projects migration', () => {
    ...
    it('(React standalone) should set root project config to app and e2e app and migrate when another lib is added', () => {
      ...
      runCLI(
        `generate @nx/js:lib libs/${mylib} --linter=eslint --unitTestRunner=jest`
      );
